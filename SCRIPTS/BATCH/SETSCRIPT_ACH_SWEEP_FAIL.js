...
if (set.statusComment && !set.status.equals("Sweep Failed")) { // we can only do anything if the set comment has the ref seq nbr of the agency contact
	...
	if (pm) {
		...
		if (rSet.next()) {
			...
			for (var i in achTable) {
				if (String(achTable[i]["SETID"]).equals(String(SetID)) && String(achTable[i]["SetStatus"]).equals(String("Swept"))) {
						logDebug("Found a row in the SET_STATUS table that matches");
						if (!lastSwept) {
							sweepRow = achTable[i];
			...
			if (sweepRow) {
				sweepRow["SetStatus"] = "Sweep Failed";
				sweepRow["DateTime"] = dateAdd(null,0);
				sweepRow["SETPreviousStatus"] = "Swept";
	...
	logDebug("Can't Fail the Sweep.  Set Status must not be 'Sweep Failed' and there must be a comment containing the agent number");
	}
			
	
aa.env.setValue("ScriptReturnCode","0");
aa.env.setValue("ScriptReturnMessage", "Update Set successful - Permit Issuance Script agent : " + debug);  
